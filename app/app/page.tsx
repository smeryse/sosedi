...
const quickActions = [
  ...
  { label: "Мои группы", href: "/app/group", icon: UsersRound },
  { label: "Калькулятор расходов", href: "/app/budget", icon: WalletCards },
];

function SectionHeading({ title, href }: { title: string; href: string }) {
  return (
    <div className="mb-3.5 flex items-center justify-between gap-4">
      <h2 className="text-[16px] font-extrabold tracking-[-0.02em] text-[#111111]">{title}</h2>
...
function DashboardMap() {
  ...
  return (
    <section>
      ...
      <div className="grid min-h-[300px] overflow-hidden rounded-[24px] border border-[#E5E5E0] bg-white shadow-sm md:grid-cols-[200px_minmax(0,1fr)]">
          ...
          <div className="space-y-4">
            {districts.map(([name, price], index) => (
              <Link key={name} href="/app/housing" className="group block">
                <p className="flex items-center gap-2 text-[12px] font-extrabold text-[#111111]">
...
function DashboardPropertyCard({ property }: { property: (typeof demoProperties)[number] }) {
  return (
    <article className="group flex flex-col overflow-hidden rounded-[20px] border border-[#E5E5E0] bg-white p-3 shadow-sm transition-transform hover:-translate-y-0.5">
      <Link href={`/app/housing/${property.id}`} className="relative h-[136px] overflow-hidden rounded-[14px]">
      ...
      <div className="flex flex-1 flex-col pt-3">
        <p className="text-[14px] font-extrabold text-[#111111]">{formatRubles(property.price)} <span className="text-[11px] font-normal text-[#6B6F66]">/ мес</span></p>
        <h3 className="mt-1 line-clamp-1 text-[11.5px] font-bold text-[#111111]">{property.title}</h3>
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
      <div className="min-w-0 space-y-6">
        <section className="flex flex-col justify-between gap-2 sm:flex-row sm:items-end">
          <div>
            <h1 className="text-[28px] font-extrabold tracking-tight text-[#111111] sm:text-[34px]">
              Доброе утро, Анна! 👋
            </h1>
          ...
          {quickActions.map((action) => {
            ...
            return (
              <Link
                key={action.href}
                href={action.href}
                className={`group flex min-h-[112px] flex-col justify-between rounded-[22px] p-4.5 transition-transform hover:-translate-y-0.5 ${
                  action.featured
      ...
      <aside className="space-y-4">
        <section className="rounded-[24px] border border-[#E5E5E0] bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between gap-2">
            <h2 className="text-[14px] font-extrabold text-[#111111]">Ваша совместимость</h2>
          ...
          <p className="mt-1 text-[38px] font-black leading-none tracking-tight text-[#8FB000]">93%</p>
          <p className="mt-1.5 text-[10px] font-medium text-[#6B6F66]">Это отличный результат!</p>
          ...
          <div className="mt-4.5 space-y-2.5">
            {[
              ...
              <div key={label as string} className="grid grid-cols-[1fr_80px_30px] items-center gap-2 text-[10px]">
                <span className="text-[#6B6F66]">{label as string}</span>
                <span className="h-1.5 rounded-full bg-[#F4F4F0]">
                  <span className="block h-full rounded-full bg-[#B3DB00]" style={{ width: `${score}%` }} />
                </span>
                <span className="text-right font-bold text-[#111111]">{score}%</span>
              </div>
            ))}
          </div>

          <Link href="/app/compatibility" className="mt-5 flex h-9 w-full items-center justify-center rounded-full bg-[#F4F4F0] text-[11px] font-bold text-[#111111] transition-colors hover:bg-[#EBF7B6]">
            Смотреть профиль
          </Link>
        </section>
        ...
        <section className="rounded-[24px] border border-[#E5E5E0] bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <h2 className="text-[14px] font-extrabold text-[#111111]">AI помощник</h2>
            <span className="rounded-full bg-[#F4F4F0] px-2 py-0.5 text-[9px] font-extrabold text-[#6B6F66]">BETA</span>
          </div>
          <div className="mt-3.5 flex gap-3 rounded-[16px] bg-[#F4F4F0] p-3">
            <span className="grid size-8 shrink-0 place-items-center rounded-full bg-[#B3DB00] text-[#111111]">
              <Sparkles className="size-4" />
            </span>
            <div>
              <p className="text-[11px] font-bold text-[#111111]">Привет! Я ваш AI-помощник.</p>
              <p className="mt-1 text-[10px] leading-4 text-[#6B6F66]">Спросите меня о совместимости, конфликтах, быте или поиске жилья.</p>
            </div>
          </div>
        ...
        <section className="rounded-[24px] border border-[#E5E5E0] bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <h2 className="text-[14px] font-extrabold text-[#111111]">Мои группы</h2>
            <Link href="/app/group" className="text-[10.5px] font-bold text-[#6B6F66] hover:text-[#111111]">Смотреть все</Link>
          </div>
          <div className="mt-3.5 space-y-3">
            <Link href="/app/group" className="flex items-center gap-3 rounded-[16px] border border-[#E5E5E0] p-2.5 transition-colors hover:bg-[#F4F4F0]">
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate text-[11px] font-extrabold text-[#111111]">Ищем третьего в двушку</p>
                <p className="text-[9.5px] text-[#6B6F66]">2 участника · Центр</p>
              </div>
              <span className="rounded-full bg-[#EBF7B6] px-2 py-0.5 text-[9px] font-extrabold text-[#7B9E00]">
                89%
              </span>
            </Link>
            ...
            <Link href="/app/group" className="flex items-center gap-3 rounded-[16px] border border-[#E5E5E0] p-2.5 transition-colors hover:bg-[#F4F4F0]">
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate text-[11px] font-extrabold text-[#111111]">Квартира у парка</p>
                <p className="text-[9.5px] text-[#6B6F66]">3 участника · Фестивальный</p>
              </div>
              <span className="rounded-full bg-[#EBF7B6] px-2 py-0.5 text-[9px] font-extrabold text-[#7B9E00]">
                91%
              </span>
          ...
          <Link href="/app/group/create" className="mt-4 flex h-9 w-full items-center justify-center rounded-full bg-[#F4F4F0] text-[11px] font-bold text-[#111111] transition-colors hover:bg-[#EBF7B6]">
            Создать группу
          </Link>
        </section>
      </aside>
