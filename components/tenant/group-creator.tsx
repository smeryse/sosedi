...
export function GroupCreator() {
  ...
  const submit = async () => { setError(""); const parsed = groupInputSchema.safeParse({ name, targetBudget: Number(budget), moveInDate }); if (!parsed.success) { setError("Проверьте название, бюджет и дату въезда."); return; } setSaving(true); try { await new DemoRepository().createGroup(parsed.data); router.push("/app/group"); router.refresh(); } catch (cause) { setError(cause instanceof Error ? cause.message : "Не удалось создать группу"); } finally { setSaving(false); } };
